...
public class OPC implements Runnable
{
  ...
  void setLocalPlayback(boolean onoff) {
   this.localPlayback = onoff; 
  }
  void enableOPCRecording(boolean onoff) {
  recordingEnabled = onoff;
  }
  ...
  void draw()
  {
    ...
    // write to local file
    if (recordingEnabled) opcrecorder.writePixelFrame(packetData);
    
    if (!localPlayback) writePixels();
