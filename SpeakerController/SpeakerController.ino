...
void irReceive() 
{
  if (IrReceiver.decode()) {
    ...
    if (IrReceiver.decodedIRData.address == 0x7D02) { // remote address        
    // code detected
      if (powerState == 1) { // powered on state
      	if (IrReceiver.decodedIRData.command == 0xA2) { // select
          writeRelays = 1;
          functionSelect(99); 
      	}      
      	if (IrReceiver.decodedIRData.command == 0x5F) { // up button
          functionSelect(0);
      	}	  
      	if (IrReceiver.decodedIRData.command == 0xA3) { // down button
          functionSelect(1);
      	}                                      
      }   
    }
