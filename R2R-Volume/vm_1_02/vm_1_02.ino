...
const char	fl_st_romVersion[]	PROGMEM  = "Volu-Master " PROG_VER;
...
#ifdef USE_SPDIF
	if (option_delta2_board_count == 3) {
		// turn all the software-settable leds ON
		pcf.write(delta_i2c_addr[I2C_SPDIF_ADDR_SLOT], B00000000);
	}
#endif
...
setup(void)
{
	...
	// software_serial.println("Volu-Master " PROG_VER);
	...
	Serial.println("Volu-Master " PROG_VER);
...
read_eeprom_oper_values(void)
{
	...
	output_selector = EEPROM.read(EEPROM_OUTPUT_SEL);
	output_selector_mask = EEPROM.read(EEPROM_OUTPUT_SEL_MASK);
	output_mode_radio_or_toggle = EEPROM.read(EEPROM_OUTPUT_RB_TB);
